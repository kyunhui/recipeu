...
export default function CookModeAudioPage() {
  ...
  const [messages, setMessages] = useState(() => {
    ...
    return [
      { id: "welcome", type: "ai", text: randomMsg, status: "done" },
      // [요구사항 2] 추가 안내 메시지 자동 노출
      { id: "info_1", type: "ai", text: "재료 대체부터 실수 해결까지, 퓨가 도와드릴게요.", status: "done" },
      { id: "info_2", type: "ai", text: "목소리로 요리 단계를 자유롭게 조절해보세요.", status: "done" }
    ];
  });
  ...
  function resetIdleHintTimer() {
    ...
    idleHintTimerRef.current = setTimeout(() => {
      idleHintTimerRef.current = null;
      // 가이드 메시지 중 랜덤 선택
      const randomGuide = GUIDE_MESSAGES[Math.floor(Math.random() * GUIDE_MESSAGES.length)];
      
      // hint ID는 'hint_'로 시작하도록 하여 history 필터링에서 식별
      appendMessage({
        id: `hint_${Date.now()}`,
        type: "ai",
        text: randomGuide,
  ...
  async function processSTT(audioBlob) {
    ...
    try {
      ...
      // 텍스트 없음 -> User Thinking 모두 제거 후 종료
      if (!text) {
        console.log("[processSTT] 텍스트 없음 → user thinking 제거");
        setMessages((prev) => prev.filter((m) => !(m.type === "user" && m.status === "thinking")));
        userThinkingMsgIdRef.current = null;
  ...
  async function processTextWithBackend(userText) {
    ...
    const neighborStepText = neighborIndices
      ...
      .filter(Boolean)
      .join(" / ");
    const recipeContext = neighborStepText ? `인접 단계: ${neighborStepText}` : "";
    ...
    const history = messages
      ...
      .filter((m) => !String(m.id).startsWith("info_")) // 초기 안내 제외
      .filter((m) => !String(m.id).startsWith("hint_")) // 가이드 메시지 제외
      .slice(-6)
      .map((m) => ({
